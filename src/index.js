...
bank.on('connection', async socket => {
    ...
    socket.on('deposit', async id => {
        ...
        if (String(deposit.steamid) !== String(user.steamid)) {
            bank.to(user.steamid).emit('err', TXT.TRADE_ABORT_ERROR_GET_DETAILS('deposit'));
            return;
        }
        if (deposit && deposit.trade_link) {
            ...
            offer.getUserDetails((err, me, them) => {
                if (err) {
                    onErrorGetUserDetails('deposit', err, user);
                    return;
                }
                if (process.env.NODE_ENV !== 'devel') {
                    if (hasEscrow(them)) {
                        bank.to(user.steamid).emit('err', TXT.TRADE_ABORT_ESCROW('deposit'));
                        return;
                    }
                }
                if (hasProbation(them)) {
                    bank.to(user.steamid).emit('err', TXT.TRADE_ABORT_PROBATION('deposit'));
                    return;
                }
                ...
                offer.send(async (err, status) => {
                    if (err) onErrorSendTrade('deposit', err, status, offer, user);
                        ...
                        bank.to(user.steamid).emit('err', TXT.TRADE_ERROR_SEND('deposit'));
                    }
                });
            });
        }
    ...
    socket.on('withdrawal', async id => {
        ...
        offer.getUserDetails((err, me, them) => {
            ...
            if (hasEscrow(them)) {
                bank.to(user.steamid).emit('err', TXT.TRADE_ABORT_ESCROW('withdrawal'));
                return;
            }
            if (hasProbation(them)) {
                bank.to(user.steamid).emit('err', TXT.TRADE_ABORT_PROBATION('withdrawal'));
                return;
            }
            fn.addMyItems(withdrawal.items, offer);
            offer.setMessage(TXT.SUBTRACT_CHIPS(withdrawal.items_value, user.player));
            offer.send((err, status) => {
                if (err) onErrorSendTrade('withdrawal', err, status, offer, user);
                else if (status === 'pending') {
                    console.log(`Offer #${offer.id} sent, but requires confirmation`);
                    steam.community.acceptConfirmationForObject(steam.identitySecret, offer.id, async err => {
                        if (err) {
                            console.error(err);
                            bank.to(user.steamid).emit('err', TXT.TRADE_CONFIRMATION_PENDING('withdrawal'));
                        }
                        else {
